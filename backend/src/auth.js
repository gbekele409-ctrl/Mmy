...
/*  side before trusting any user info in it.                             */
...
async function requireAuth(req, res, next) {
  try {
    ...
    const { rows } = await query(
      'SELECT id, username, role, balance, is_active, telegram_id, telegram_first_name, telegram_photo_url FROM users WHERE id = $1',
      [payload.sub]
    );
    const user = rows[0];
