...
async function requireAuth(req, res, next) {
  try {
    ...
    const { rows } = await query(
      'SELECT id, username, role, balance, bonus_balance, wagering_required, wagering_target_total, channels_verified, is_active, telegram_id, telegram_first_name, telegram_photo_url, telegram_phone, referral_code FROM users WHERE id = $1',
      [payload.sub]
    );
    const user = rows[0];
...
router.get('/verify-channels', requireAuth, async (req, res, next) => {
  try {
    ...
    await query('UPDATE users SET channels_verified = TRUE WHERE id = $1', [req.user.id]);
