...
async function requireAuth(req, res, next) {
  try {
    ...
    const { rows } = await query(
      'SELECT id, username, role, balance, bonus_balance, wagering_required, is_active, telegram_id, telegram_first_name, telegram_photo_url, telegram_phone, referral_code FROM users WHERE id = $1',
      [payload.sub]
    );
    const user = rows[0];
