...
const STAKE_AMOUNT_CENTS = 1000;        // 10 ETB per cartela
const PLATFORM_FEE_CENTS = 200;         // 2 ETB per cartela kept by the platform
const PAYOUT_PER_CARTELA_CENTS = STAKE_AMOUNT_CENTS - PLATFORM_FEE_CENTS; // 8 ETB per cartela into the winner pool
...
async function ensureRoundOpen() {
  ...
  await query(
    ...
    [round_id, STAKE_AMOUNT_CENTS, PLATFORM_FEE_CENTS]
  );
...
async function finishRoundWithWinners(winners) {
  ...
  try {
    ...
    const fullPrizePool = totalCartelas * PAYOUT_PER_CARTELA_CENTS;
...
function publicRoundState() {
  ...
  return {
    ...
    stake_amount: STAKE_AMOUNT_CENTS / 100,
  };
}
...
router.post('/stake', requireAuth, stakeValidation, handleValidation, async (req, res, next) => {
  ...
  try {
    ...
    const totalStake = cartelaNumbers.length * STAKE_AMOUNT_CENTS;
    const totalFee = cartelaNumbers.length * PLATFORM_FEE_CENTS;
    ...
    for (const cartelaNumber of cartelaNumbers) {
      await client.query(
        `INSERT INTO bingo_stakes (round_id, user_id, cartela_number, stake_amount, platform_fee_amount)
         VALUES ($1, $2, $3, $4, $5)`,
        [currentRound.round_id, req.user.id, cartelaNumber, STAKE_AMOUNT_CENTS, PLATFORM_FEE_CENTS]
      );
    }
