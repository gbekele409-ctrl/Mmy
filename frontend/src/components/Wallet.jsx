...
import { requestDeposit, requestWithdraw, getMyTransactions, getBalance, getSocket } from '../api.js';
...
export default function Wallet() {
  ...
  useEffect(() => {
    ...
    // Instant push path: the backend emits this the moment a
    // balance-changing action commits (deposit/withdraw approval, a
    // game payout, a bingo win, a cashback claim) instead of waiting for
    // the next 10s poll. The poll above stays on as a fallback in case
    // the socket drops.
    const socket = getSocket();
    socket.on('wallet:balance_updated', (payload) => {
      const fresh = payload.balance;
      if (fresh !== balanceRef.current) {
        setBalanceUpdating(true);
        setDisplayBalance(fresh);
        balanceRef.current = fresh;
      ...
      // A balance-changing event almost always means a new/updated
      // transaction too (approval, payout, claim) - refresh history so
      // it shows up without the user needing to switch tabs.
      loadTransactions();
    });
  ...
  }, [refreshBalance, loadTransactions, updateBalance]);
        ...
        {!wageringComplete && (
          <div className="bonus-banner">
            ...
            <span className="bonus-banner-text">
              You received a <strong>{(wageringTarget / WAGERING_MULTIPLIER).toFixed(0)} ETB</strong> registration bonus. Wager{' '}
